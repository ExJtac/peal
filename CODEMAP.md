...
| `test/vm/{Vagrantfile,README.md}` · `.gitattributes` | Cross-platform disposable Debian 13 VM to test `install.sh` end-to-end (VirtualBox + Vagrant → `vagrant up`). Defaults to the GitHub one-liner; `PEAL_SOURCE=local` installs the working tree. `.gitattributes` forces LF so scripts survive a Windows checkout. Mirrors the Mac-only `asterisk/lima/pbx.yaml` |
